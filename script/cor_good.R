...
#缺失值填补
info_good$th_due_rate[is.na(info_good$th_due_rate)]=0
info_good$sv_due_rate[is.na(info_good$sv_due_rate)]=0
info_good$th_kp_rate[is.na(info_good$th_kp_rate)]=0
info_good$sx_kp_rate[is.na(info_good$sx_kp_rate)]=0
info_good$sx_eff_rate[is.na(info_good$sx_eff_rate)]=0
info_good$th_eff_rate[is.na(info_good$th_eff_rate)]=0
info_good$return_to_loan_term[is.na(info_good$return_to_loan_term)]=0
info_good$bal_prin2 <-impute(info_good$bal_prin2,median)
info_good$return_to_loan_term <-impute(info_good$return_to_loan_term,mean)
info_good$bal_percent <- impute(info_good$bal_percent,mean)

info_good$last_visit_days[is.na(info_good$last_visit_days)]=max(info_good$last_visit_days,na.rm = T)
info_good$days_last_p[is.na(info_good$days_last_p)]=max(info_good$days_last_p,na.rm = T)

info_good$no_call_days[is.na(info_good$no_call_days)]=0
info_good$th_count_rate <- info_good$count_th_kp/info_good$count_th_ptp
info_good$th_count_rate[is.na(info_good$th_count_rate)]=0
info_good$sx_count_rate <- info_good$count_sx_kp/info_good$count_sx_ptp
info_good$sx_count_rate[is.na(info_good$sx_count_rate)]=0

info_good$last_activitive[info_good$last_activitive=='预催收']=1
info_good$last_activitive[info_good$last_activitive=='手工电催']=2
info_good$last_activitive[info_good$last_activitive=='反欺诈调查']=3
info_good$last_activitive[is.na(info_good$last_activitive)]=4

info_good$thdays_payoff_times[is.na(info_good$thdays_payoff_times)] = 0
info_good$svdays_payoff_times[is.na(info_good$svdays_payoff_times)] = 0
info_good$no_call_days[is.na(info_good$no_call_days)]=0
#last_code还未编码
info_good$prd <-with(info_good,
...
info_good$last_code <-with(info_good,
...
num <- info_good[,c('order_connected','bal_prin2','due_amount','list_bal_prin2','th_due_rate','sv_due_rate',
...
for(i in 2:times){
  ...
  n <- nrow(table2)
  chisq_p <- cbind(chisq_p,chisq.test(table2,simulate.p.value = T,B = 2000)$p.value)
